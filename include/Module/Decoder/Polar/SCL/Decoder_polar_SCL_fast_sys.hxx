...
template<>
inline signed char
sat_m(const signed char m)
{
    return spu::tools::saturate<signed char>(m, -128, 63);
}
...
template<>
inline void
normalize_scl_metrics(std::vector<short>& metrics, const int L)
{
...
template<>
inline void
normalize_scl_metrics(std::vector<signed char>& metrics, const int L)
{
    auto min = *std::min_element(metrics.begin(), metrics.begin() + L);
...
Decoder_polar_SCL_fast_sys<B, R, API_polar>::Decoder_polar_SCL_fast_sys(const int& K,
                                                                        const int& N,
                                                                        const int& L,
...
template<typename B, typename R, class API_polar>
Decoder_polar_SCL_fast_sys<B, R, API_polar>::Decoder_polar_SCL_fast_sys(
  const int& K,
  const int& N,
  const int& L,
  const std::vector<bool>& frozen_bits,
...
Decoder_polar_SCL_fast_sys<B, R, API_polar>::clone() const
{
...
template<typename B, typename R, class API_polar>
void
Decoder_polar_SCL_fast_sys<B, R, API_polar>::set_frozen_bits(const std::vector<bool>& fb)
{
...
Decoder_polar_SCL_fast_sys<B, R, API_polar>::get_frozen_bits() const
{
...
template<typename B, typename R, class API_polar>
void
Decoder_polar_SCL_fast_sys<B, R, API_polar>::_decode(const R* Y_N)
{
...
template<typename B, typename R, class API_polar>
int
Decoder_polar_SCL_fast_sys<B, R, API_polar>::_decode_siho(const R* Y_N, B* V_K, const size_t frame_id)
{
...
template<typename B, typename R, class API_polar>
int
Decoder_polar_SCL_fast_sys<B, R, API_polar>::_decode_siho_cw(const R* Y_N, B* V_N, const size_t frame_id)
{
    if (!API_polar::isAligned(Y_N))
        throw spu::tools::runtime_error(__FILE__, __LINE__, __func__, "'Y_N' is misaligned memory.");
...
template<typename B, typename R, class API_polar>
void
Decoder_polar_SCL_fast_sys<B, R, API_polar>::recursive_decode(const R* Y_N,
                                                              const int off_l,
                                                              const int off_s,
                                                              const int rev_depth,
                                                              int& node_id)
{
    ...
    const bool is_terminal_pattern = (node_type == tools::polar_node_t::RATE_0) ||
                                     ...
                                     (node_type == tools::polar_node_t::REP) || (node_type == tools::polar_node_t::SPC);
        ...
        switch (node_type)
        {
            case tools::polar_node_t::STANDARD:
                for (auto i = 0; i < n_active_paths; i++)
                {
                    ...
                    const auto parent = l[path_2_array[path][rev_depth]].data();
                    const auto child = l[up_ref_array_idx(path, rev_depth - 1)].data();
                    API_polar::g(parent + off_l,
                                 parent + off_l + n_elm_2,
                                 s[path].data() + off_s,
                                 child + off_l + n_elmts,
                                 n_elm_2);
                }
    ...
    else // leaf node
    {
        ...
        switch (node_type)
        {
            case tools::polar_node_t::RATE_0:
                update_paths_r0(rev_depth, off_l, off_s, n_elmts);
                break;
            case tools::polar_node_t::REP:
                update_paths_rep(rev_depth, off_l, off_s, n_elmts);
                break;
            case tools::polar_node_t::RATE_1:
                update_paths_r1(rev_depth, off_l, off_s, n_elmts);
                break;
            case tools::polar_node_t::SPC:
                update_paths_spc(rev_depth, off_l, off_s, n_elmts);
                break;
            default:
                break;
        }
...
template<typename B, typename R, class API_polar>
void
Decoder_polar_SCL_fast_sys<B, R, API_polar>::_store(B* V_K) const
{
...
template<typename B, typename R, class API_polar>
void
Decoder_polar_SCL_fast_sys<B, R, API_polar>::_store_cw(B* V_N) const
{
...
void
Decoder_polar_SCL_fast_sys<B, R, API_polar>::update_paths_r0(const int off_l, const int off_s)
{
    if (n_active_paths > 1)
        for (auto i = 0; i < n_active_paths; i++)
        {
...
template<typename B, typename R, class API_polar>
void
Decoder_polar_SCL_fast_sys<B, R, API_polar>::update_paths_r1(const int r_d,
                                                             const int off_l,
                                                             const int off_s,
                                                             const int n_elmts)
{
...
template<int REV_D, int N_ELMTS>
void
Decoder_polar_SCL_fast_sys<B, R, API_polar>::update_paths_r1(const int off_l, const int off_s)
{
    if (REV_D == 0)
...
template<typename B, typename R, class API_polar>
void
Decoder_polar_SCL_fast_sys<B, R, API_polar>::flip_bits_r1(const int old_path,
                                                          const int new_path,
                                                          const int dup,
                                                          const int off_s,
                                                          const int /*n_elmts*/)
{
    ...
    switch (dup)
    {
        case 0:
            ...
            break;
        case 1:
            s[new_path][off_s + bit_flips[2 * old_path + 0]] =
              !s[old_path][off_s + bit_flips[2 * old_path + 0]] ? b : 0;
            break;
        case 2:
            s[new_path][off_s + bit_flips[2 * old_path + 1]] =
              !s[old_path][off_s + bit_flips[2 * old_path + 1]] ? b : 0;
            break;
        case 3:
            s[new_path][off_s + bit_flips[2 * old_path + 0]] =
              !s[old_path][off_s + bit_flips[2 * old_path + 0]] ? b : 0;
...
template<typename B, typename R, class API_polar>
void
Decoder_polar_SCL_fast_sys<B, R, API_polar>::update_paths_rep(const int r_d,
                                                              const int off_l,
                                                              const int off_s,
                                                              const int n_elmts)
{
...
template<int REV_D, int N_ELMTS>
void
Decoder_polar_SCL_fast_sys<B, R, API_polar>::update_paths_rep(const int off_l, const int off_s)
{
    constexpr B b = spu::tools::bit_init<B>();
...
template<typename B, typename R, class API_polar>
void
Decoder_polar_SCL_fast_sys<B, R, API_polar>::update_paths_spc(const int r_d,
                                                              const int off_l,
                                                              const int off_s,
                                                              const int n_elmts)
{
    ...
    if (n_elmts == 4)
    {
        for (auto i = 0; i < n_active_paths; i++)
        {
            ...
            if (L > 2)
            {
                metrics_vec[2][n_cands * path + 4] =
                  sat_m<R>(sat_m<R>(metrics_vec[2][n_cands * path + 0] + pen1) + pen2);
                metrics_vec[2][n_cands * path + 5] =
                  sat_m<R>(sat_m<R>(metrics_vec[2][n_cands * path + 0] + pen1) + pen3);
                metrics_vec[2][n_cands * path + 6] =
                  sat_m<R>(sat_m<R>(metrics_vec[2][n_cands * path + 0] + pen2) + pen3);
                metrics_vec[2][n_cands * path + 7] =
                  sat_m<R>(sat_m<R>(metrics_vec[2][n_cands * path + 1] + pen2) + pen3);
            }
        }
    }
    else
...
template<int REV_D, int N_ELMTS>
void
Decoder_polar_SCL_fast_sys<B, R, API_polar>::update_paths_spc(const int off_l, const int off_s)
{
    // the number of candidates to generate per list
    const auto n_cands = L <= 2 ? 4 : 8;
...
template<typename B, typename R, class API_polar>
void
Decoder_polar_SCL_fast_sys<B, R, API_polar>::flip_bits_spc(const int old_path,
                                                           const int new_path,
                                                           const int dup,
                                                           const int off_s,
                                                           const int /*n_elmts*/)
{
    ...
    switch (dup)
    {
        case 0:
            if (!is_even[old_path])
                s[new_path][off_s + bit_flips[4 * old_path + 0]] =
                  s[old_path][off_s + bit_flips[4 * old_path + 0]] ? 0 : b;
            break;
        case 1:
            if (is_even[old_path])
                s[new_path][off_s + bit_flips[4 * old_path + 0]] =
...
template<typename B, typename R, class API_polar>
void
Decoder_polar_SCL_fast_sys<B, R, API_polar>::delete_path(int path_id)
{
...
Decoder_polar_SCL_fast_sys<B, R, API_polar>::select_best_path(const size_t /*frame_id*/)
{
    best_path = -1;
    for (auto i = 0; i < n_active_paths; i++)
        if (best_path == -1 || metrics[paths[i]] < metrics[best_path]) best_path = paths[i];
...
template<typename B, typename R, class API_polar>
int
Decoder_polar_SCL_fast_sys<B, R, API_polar>::up_ref_array_idx(const int path, const int r_d)
{
    ...
    if (n_array_ref[old_array][r_d] > 1)
    {
        ...
        auto new_array = 0;
        while (n_array_ref[new_array][r_d])
            new_array++;
...
template<typename B, typename R, class API_polar>
void
Decoder_polar_SCL_fast_sys<B, R, API_polar>::erase_bad_paths()
{
    // erase bad paths
    auto k = 0;
