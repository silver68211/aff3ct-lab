...
Decoder_polar_PAC_SCL_naive<B, R>::Decoder_polar_PAC_SCL_naive(
  ...
  const std::vector<std::vector<std::function<R(const std::vector<R>& LLRs, const std::vector<B>& bits)>>>& lambdas)
  ...
  , lambdas(code.get_kernel_matrices().size())
{
    ...
    for (auto i = 0; i < L; i++)
    {
        this->polar_trees.push_back(tools::Generic_tree_metric<Contents_PAC_SCL<B, R>, R>(sequence, metric_init));
    ...
    std::cout << "In function: " << __func__ << " at line: " << __LINE__ << " in file: " << __FILE__ << std::endl;
}

template<typename B, typename R>
Decoder_polar_PAC_SCL_naive<B, R>::Decoder_polar_PAC_SCL_naive(const int& K,
                                                               const int& N,
                                                               const int& L,
                                                               const tools::Polar_code& code,
                                                               const std::vector<bool>& frozen_bits)
  : Decoder_polar_PAC_SCL_naive<B, R>(K, N, L, code, frozen_bits, {})
{
}
...
template<typename B, typename R>
Decoder_polar_PAC_SCL_naive<B, R>*
Decoder_polar_PAC_SCL_naive<B, R>::clone() const
{
...
template<typename B, typename R>
void
Decoder_polar_PAC_SCL_naive<B, R>::deep_copy(const Decoder_polar_PAC_SCL_naive<B, R>& m)
{
...
template<typename B, typename R>
void
Decoder_polar_PAC_SCL_naive<B, R>::recursive_deep_copy(const tools::Generic_node<Contents_PAC_SCL<B, R>>* nref,
                                                       tools::Generic_node<Contents_PAC_SCL<B, R>>* nclone)
{
...
template<typename B, typename R>
void
Decoder_polar_PAC_SCL_naive<B, R>::set_frozen_bits(const std::vector<bool>& fb)
{
...
Decoder_polar_PAC_SCL_naive<B, R>::get_frozen_bits() const
{
...
template<typename B, typename R>
void
Decoder_polar_PAC_SCL_naive<B, R>::_load(const R* Y_N)
{
...
template<typename B, typename R>
void
Decoder_polar_PAC_SCL_naive<B, R>::_decode(const size_t frame_id)
{
...
template<typename B, typename R>
int
Decoder_polar_PAC_SCL_naive<B, R>::_decode_siho(const R* Y_N, B* V_K, const size_t frame_id)
{
...
template<typename B, typename R>
int
Decoder_polar_PAC_SCL_naive<B, R>::_decode_siho_cw(const R* Y_N, B* V_N, const size_t frame_id)
{
    //	auto t_load = std::chrono::steady_clock::now(); // -----------------------------------------------------------
    // LOAD
    this->_load(Y_N);
...
template<typename B, typename R>
void
Decoder_polar_PAC_SCL_naive<B, R>::_store(B* V, bool coded) const
{
...
template<typename B, typename R>
void
Decoder_polar_PAC_SCL_naive<B, R>::recursive_compute_llr(tools::Generic_node<Contents_PAC_SCL<B, R>>* node_cur,
                                                         int depth)
{
    auto node_father = node_cur->get_father();

    if (depth != 0) recursive_compute_llr(node_father, --depth);
    ...
    if (!node_cur->is_root())
    {
        ...
        {
            for (auto l = 0; l < kern_size; l++)
                LLRs[l] = node_father->get_c()->l[l * n_kernels + k];
            for (auto c = 0; c < child; c++)
                bits[c] = node_father->get_children()[c]->get_c()->s[k];
...
template<typename B, typename R>
void
Decoder_polar_PAC_SCL_naive<B, R>::recursive_propagate_sums(const tools::Generic_node<Contents_PAC_SCL<B, R>>* node_cur)
{
    if (!node_cur->is_leaf())
    {
    ...
    if (!node_cur->is_root() && ((size_t)node_cur->get_child_id() == node_cur->get_father()->get_children().size() - 1))
        this->recursive_propagate_sums(node_cur->get_father());
}

template<typename B, typename R>
void
Decoder_polar_PAC_SCL_naive<B, R>::recursive_duplicate_tree_llr(tools::Generic_node<Contents_PAC_SCL<B, R>>* node_a,
                                                                tools::Generic_node<Contents_PAC_SCL<B, R>>* node_b)
{
...
template<typename B, typename R>
void
Decoder_polar_PAC_SCL_naive<B, R>::recursive_duplicate_tree_sums(
  tools::Generic_node<Contents_PAC_SCL<B, R>>* node_a,
  tools::Generic_node<Contents_PAC_SCL<B, R>>* node_b,
  tools::Generic_node<Contents_PAC_SCL<B, R>>* node_caller)
{
    if (!node_a->is_leaf())
        for (size_t c = 0; c < node_a->get_children().size() - 1; c++)
        {
            auto child_a = node_a->get_children()[c];
            if (child_a != node_caller) node_b->get_children()[c]->get_c()->s = child_a->get_c()->s;
        }
...
template<typename B, typename R>
void
Decoder_polar_PAC_SCL_naive<B, R>::duplicate_path(
  int path,
  int leaf_index,
  std::vector<std::vector<tools::Generic_node<Contents_PAC_SCL<B, R>>*>> leaves_array)
{
    std::vector<tools::Generic_node<Contents_PAC_SCL<B, R>>*> path_leaves, newpath_leaves;
    int newpath = 0;
    ...
    polar_trees[newpath].set_path_metric(
      tools::phi<B, R>(polar_trees[path].get_path_metric(), leaves_array[path][leaf_index]->get_c()->l[0], (B)1));

    leaves_array[path][leaf_index]->get_c()->s[0] = (B)0;
    polar_trees[path].set_path_metric(
      tools::phi<B, R>(polar_trees[path].get_path_metric(), leaves_array[path][leaf_index]->get_c()->l[0], (B)0));
}

template<typename B, typename R>
void
Decoder_polar_PAC_SCL_naive<B, R>::select_best_path(const size_t /*frame_id*/)
{
    int best_path = 0;
...
template<typename B, typename R>
void
Decoder_polar_PAC_SCL_naive<B, R>::recursive_allocate_nodes_contents(
  tools::Generic_node<Contents_PAC_SCL<B, R>>* node_curr,
...
template<typename B, typename R>
void
Decoder_polar_PAC_SCL_naive<B, R>::recursive_initialize_frozen_bits(
  const tools::Generic_node<Contents_PAC_SCL<B, R>>* node_curr,
  const std::vector<bool>& frozen_bits)
{
...
template<typename B, typename R>
void
Decoder_polar_PAC_SCL_naive<B, R>::recursive_store(const tools::Generic_node<Contents_PAC_SCL<B, R>>* node_curr,
                                                   B* V_K,
                                                   int& k) const
{
    if (!node_curr->is_leaf()) // stop condition
...
template<typename B, typename R>
void
Decoder_polar_PAC_SCL_naive<B, R>::recursive_deallocate_nodes_contents(
  tools::Generic_node<Contents_PAC_SCL<B, R>>* node_curr)
{
    for (auto c : node_curr->get_children())
...
template class aff3ct::module::Decoder_polar_PAC_SCL_naive<B_8, Q_8>;
template class aff3ct::module::Decoder_polar_PAC_SCL_naive<B_16, Q_16>;
template class aff3ct::module::Decoder_polar_PAC_SCL_naive<B_32, Q_32>;
template class aff3ct::module::Decoder_polar_PAC_SCL_naive<B_64, Q_64>;
#else
template class aff3ct::module::Decoder_polar_PAC_SCL_naive<B, Q>;
#endif
// ==================================================================================== explicit template instantiation
