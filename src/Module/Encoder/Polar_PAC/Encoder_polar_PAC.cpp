  ...
  , u(code.get_biggest_kernel_size())
{
    ...
    this->conv_reg = { 1, 0, 1, 1, 0, 1, 1 };
    this->curState.resize(this->conv_reg.size() -1);
...
template<typename B>
void
Encoder_polar_PAC<B>::light_encode(B* X_N)
{
    ...
    for (auto s = 0; s < (int)this->code.get_stages().size(); s++)
    {
        ...
        for (auto b = 0; b < n_blocks; b++)
        {
            for (auto k = 0; k < n_kernels; k++)
            {
                ...
                apply_polar_kernel(
                  this->u.data(), this->idx.data(), this->Ke[code.get_stages()[s]].data(), X_N, kernel_size);
            }
        }
...
{
    uint8_t u = cbit & conv_reg[0];
    for(int i = 1; i < conv_reg.size(); i++){
        if(conv_reg[i] == 1){
            u = u ^ curState[i-1];
        }
    }
    for(int i = curState.size() - 1; i >= 1; i--) curState[i] = curState[i - 1];
    curState[0] = cbit;
...
template<typename B>
void
Encoder_polar_PAC<B>::ConvEncoder(B* X_N)
{
...
Encoder_polar_PAC<B>::set_frozen_bits(const std::vector<bool>& frozen_bits)
{
    ...
    for (auto n = 0; n < this->N; n++)
        if (!frozen_bits[n]) this->info_bits_pos[k++] = n;
}
