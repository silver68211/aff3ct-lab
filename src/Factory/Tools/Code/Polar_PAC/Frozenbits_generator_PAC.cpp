...
const std::string aff3ct::factory::Frozenbits_generator_PAC_name = "Frozen bits generator PAC";
const std::string aff3ct::factory::Frozenbits_generator_PAC_prefix = "fbg";

Frozenbits_generator_PAC ::Frozenbits_generator_PAC(const std::string& prefix)
  : Factory(Frozenbits_generator_PAC_name, Frozenbits_generator_PAC_name, prefix)
{
}

Frozenbits_generator_PAC*
Frozenbits_generator_PAC ::clone() const
{
    return new Frozenbits_generator_PAC(*this);
}

void
Frozenbits_generator_PAC ::get_description(cli::Argument_map_info& args) const
{
    auto p = this->get_prefix();
    const std::string class_name = "factory::Frozenbits_generator_PAC::";
    ...
    tools::add_arg(args, p, class_name + "p+gen-method", cli::Text(cli::Including_set("FILE", "GA", "GAA")));

    tools::add_arg(args, p, class_name + "p+awgn-path", cli::File(cli::openmode::read));

    tools::add_arg(args, p, class_name + "p+dump-path", cli::Folder(cli::openmode::write));
}

void
Frozenbits_generator_PAC ::store(const cli::Argument_map_value& vals)
{
    auto p = this->get_prefix();
...
void
Frozenbits_generator_PAC ::get_headers(std::map<std::string, tools::header_list>& headers, const bool full) const
{
    auto p = this->get_prefix();
...
tools::Frozenbits_generator*
Frozenbits_generator_PAC ::build(const tools::Polar_code& pc) const
{
    if (this->type == "GAA" && pc.is_mono_kernel() == 2)
        return new tools::Frozenbits_generator_GA_Arikan(this->K, this->N_cw, this->dump_channels_path);
    if (this->type == "GA")
        return new tools::Frozenbits_generator_GA(this->K, this->N_cw, pc, this->dump_channels_path);
