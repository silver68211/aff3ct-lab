...
Codec_polar_PAC<B, Q>::Codec_polar_PAC(const factory::Polar_code& pc_params,
                                     const factory::Frozenbits_generator_PAC& fb_params,
...
template<typename B, typename Q>
void
Codec_polar_PAC<B, Q>::set_frozen_bits(const std::vector<bool>& fb)
{
    if (this->fb_decoder) this->fb_decoder->set_frozen_bits(fb);
    if (this->fb_encoder) this->fb_encoder->set_frozen_bits(fb);
    if (this->fb_extractor) this->fb_extractor->set_frozen_bits(fb);
}
...
template<typename B, typename Q>
const Polar_code&
Codec_polar_PAC<B, Q>::get_code() const
{
    return *this->code;
}
