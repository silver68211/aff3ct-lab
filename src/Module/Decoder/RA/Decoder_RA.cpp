...
Decoder_RA<B, R>::Decoder_RA(const int& K, const int& N, const Interleaver<R>& interleaver, int max_iter)
...
template<typename B, typename R>
void
Decoder_RA<B, R>::deep_copy(const Decoder_RA<B, R>& m)
{
...
template<typename B, typename R>
int
Decoder_RA<B, R>::_decode_siho(const R* Y_N, B* V_K, const size_t frame_id)
{
    //	auto t_load = std::chrono::steady_clock::now(); // -----------------------------------------------------------
    // LOAD set F, B and Td at 0
...
template<typename B, typename R>
R
Decoder_RA<B, R>::check_node(R a, R b)
{
...
template<typename B, typename R>
void
Decoder_RA<B, R>::set_n_frames(const size_t n_frames)
{
