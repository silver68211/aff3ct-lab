...
enum class tmodulate : size_t
{
    X_N1,
    X_N2,
    status
};
enum class filter : size_t
{
    CP,
    Y_N1,
    Y_N2,
...
enum class demodulate_wg : size_t
{
    CP,
    H_N,
    Y_N1,
...
enum class tdemodulate_wg : size_t
{
    CP,
    H_N,
    Y_N1,
...
class Modem : public spu::module::Stateful
{
  ...
  protected:
    ...
    const int
      N_mod; /*!< Number of transmitted elements after the modulation (could be smaller, bigger or equal to N) */
  ...
  public:
    ...
    void modulate(const std::vector<B, AB>& X_N1,
    ...
    void tmodulate(const std::vector<Q, AQ>& X_N1,
                   std::vector<R, AR>& X_N2,
                   const int frame_id = -1,
                   const bool managed_memory = true);
    ...
    void filter(const std::vector<float, A>& CP,
    ...
    /*!
     * \brief Task method that demodulates a vector of noised and modulated bits/symbols (after the filtering process if
     * required).
     *
     * \param Y_N1: a vector of noised and modulated bits/symbols.
    ...
    void demodulate(const std::vector<float, A>& CP,
                    const std::vector<Q, A>& Y_N1,
                    std::vector<Q, A>& Y_N2,
                    const int frame_id = -1,
                    const bool managed_memory = true);

    void demodulate(const float* CP, const Q* Y_N1, Q* Y_N2, const int frame_id = -1, const bool managed_memory = true);
     ...
     */
    template<class AQ = std::allocator<Q>, class AR = std::allocator<R>>
    void demodulate_wg(const std::vector<float, AR>& CP,
    ...
    void demodulate_wg(const float* CP,
                       const R* H_N,
                       const Q* Y_N1,
                       Q* Y_N2,
                       const int frame_id = -1,
     ...
     */
    template<class A = std::allocator<Q>>
    void tdemodulate(const std::vector<float, A>& CP,
                     const std::vector<Q, A>& Y_N1,
    ...
    void tdemodulate(const float* CP,
                     const Q* Y_N1,
                     const Q* Y_N2,
                     Q* Y_N3,
                     const int frame_id = -1,
    ...
    /*!
     * \brief Task that demodulates a vector of noised and modulated bits/symbols (after the filtering process if
     * required).
     *
     * Used for the iterative turbo demodulation technique, this type of demodulation takes the decoder information
     * into account.
     ...
     */
    template<class AQ = std::allocator<Q>, class AR = std::allocator<R>>
    void tdemodulate_wg(const std::vector<float, AR>& CP,
                        const std::vector<R, AR>& H_N,
                        const std::vector<Q, AQ>& Y_N1,
    ...
    void tdemodulate_wg(const float* CP,
                        const R* H_N,
                        const Q* Y_N1,
                        const Q* Y_N2,
                        Q* Y_N3,
     ...
     */
    static int get_buffer_size_after_modulation(const int N,
                                                const int n_b_per_s,
                                                const int tl,
                                                const int s_factor,
                                                const bool complex);
     ...
     * \return the vector size after the modulation.
     */
    static int get_buffer_size_after_filtering(const int N,
                                               const int n_b_per_s,
                                               const int tl,
                                               const int max_wa_id,
                                               const bool complex);
  ...
  protected:
    ...
    virtual void _tdemodulate_wg(const float* CP,
                                 const R* H_N,
                                 const Q* Y_N1,
                                 const Q* Y_N2,
                                 Q* Y_N3,
