...
template<typename B>
Monitor_BFER<B>::Monitor_BFER(const int K,
                              const unsigned max_fe,
                              const unsigned max_n_frames,
  ...
  , err_hist_activated(false)
{
    ...
    this->create_codelet(p1,
                         [p1s_U, p1s_V](spu::module::Module& m, spu::runtime::Task& t, const size_t frame_id) -> int
                         {
    ...
    this->create_codelet(p2,
                         [p2s_U, p2s_V, p2s_FRA, p2s_BE, p2s_FE, p2s_BER, p2s_FER](
...
template<typename B>
Monitor_BFER<B>*
Monitor_BFER<B>::clone() const
{
    if (this->callback_fe.size())
    {
        std::stringstream message;
        message << "'callback_fe.size()' has to be equal to 0 ('callback_fe.size()' = " << this->callback_fe.size()
                << ").";
...
template<typename B>
bool
Monitor_BFER<B>::equivalent(const Monitor_BFER<B>& m, bool do_throw) const
{
    if (get_K() != m.get_K())
    {
        std::stringstream message;
        message << "'get_K()' is different than 'm.get_K()' ('get_K()' = " << get_K() << ", 'm.get_K()' = " << m.get_K()
                << ").";
        throw spu::tools::invalid_argument(__FILE__, __LINE__, __func__, message.str());
    }
...
template<typename B>
int
Monitor_BFER<B>::check_errors(const B* U, const B* V, const int frame_id, const bool managed_memory)
{
...
Monitor_BFER<B>::_check_errors(const B* U, const B* V, const size_t /*frame_id*/)
{
    ...
    for (size_t f = 0; f < this->get_n_frames(); f++)
        n_be_total += this->__check_errors(U + f * get_K(), V + f * get_K(), f);

    this->callback_check.notify();

    if (this->fe_limit_achieved()) this->callback_fe_limit_achieved.notify();
...
template<typename B>
int
Monitor_BFER<B>::check_errors2(const B* U,
                               const B* V,
                               int64_t* FRA,
                               int32_t* BE,
                               int32_t* FE,
...
template<typename B>
int
Monitor_BFER<B>::_check_errors2(const B* U,
                                const B* V,
                                int64_t* FRA,
                                int32_t* BE,
                                int32_t* FE,
                                ...
                                const size_t /*frame_id*/)
{
    ...
    for (size_t f = 0; f < this->get_n_frames(); f++)
    {
        auto n_be = this->__check_errors(U + f * get_K(), V + f * get_K(), f);
        n_be_total += n_be;
...
template<typename B>
int
Monitor_BFER<B>::__check_errors(const B* U, const B* V, const size_t frame_id)
{
    ...
    if (bit_errors_count)
    {
        ...
        if (err_hist_activated) err_hist.add_value(bit_errors_count);
...
template<typename B>
bool
Monitor_BFER<B>::fe_limit_achieved() const
{
...
template<typename B>
bool
Monitor_BFER<B>::frame_limit_achieved() const
{
...
template<typename B>
void
Monitor_BFER<B>::disable_is_done(const bool no_is_done)
{
...
template<typename B>
bool
Monitor_BFER<B>::is_done() const
{
    return !this->no_is_done && (fe_limit_achieved() || frame_limit_achieved());
}

template<typename B>
const typename Monitor_BFER<B>::Attributes&
Monitor_BFER<B>::get_attributes() const
{
    return vals;
}
...
template<typename B>
unsigned
Monitor_BFER<B>::get_max_n_frames() const
{
    return max_n_frames;
}
...
template<typename B>
unsigned
Monitor_BFER<B>::get_max_fe() const
{
    return max_fe;
}
...
template<typename B>
unsigned long long
Monitor_BFER<B>::get_n_analyzed_fra() const
{
    return vals.n_fra;
}
...
template<typename B>
bool
Monitor_BFER<B>::get_count_unknown_values() const
{
    return count_unknown_values;
}

template<typename B>
const tools::Histogram<int>&
Monitor_BFER<B>::get_err_hist() const
{
    return err_hist;
}
...
template<typename B>
void
Monitor_BFER<B>::activate_err_histogram(bool val)
{
...
template<typename B>
uint32_t
Monitor_BFER<B>::record_callback_fe(std::function<void(unsigned, int)> callback)
{
...
template<typename B>
uint32_t
Monitor_BFER<B>::record_callback_check(std::function<void(void)> callback)
{
...
template<typename B>
uint32_t
Monitor_BFER<B>::record_callback_fe_limit_achieved(std::function<void(void)> callback)
{
...
template<typename B>
bool
Monitor_BFER<B>::unrecord_callback_fe(const uint32_t id)
{
...
template<typename B>
bool
Monitor_BFER<B>::unrecord_callback_check(const uint32_t id)
{
...
template<typename B>
bool
Monitor_BFER<B>::unrecord_callback_fe_limit_achieved(const uint32_t id)
{
...
template<typename B>
void
Monitor_BFER<B>::collect(const Monitor& m, bool fully)
{
...
template<typename B>
void
Monitor_BFER<B>::collect(const Monitor_BFER<B>& m, bool fully)
{
    collect(m.get_attributes());
    if (fully) this->err_hist.add_values(m.err_hist);
}

template<typename B>
void
Monitor_BFER<B>::collect(const Attributes& v)
{
    vals += v;
}
...
template<typename B>
void
Monitor_BFER<B>::copy(const Monitor& m, bool fully)
{
...
template<typename B>
void
Monitor_BFER<B>::copy(const Monitor_BFER<B>& m, bool fully)
{
    copy(m.get_attributes());
    if (fully) this->err_hist = m.err_hist;
}

template<typename B>
void
Monitor_BFER<B>::copy(const Attributes& v)
{
    vals = v;
}
...
template<typename B>
typename Monitor_BFER<B>::Attributes&
Monitor_BFER<B>::Attributes ::operator+=(const Attributes& a)
{
    n_be += a.n_be;
    n_fe += a.n_fe;
    n_fra += a.n_fra;
...
template<typename B>
void
Monitor_BFER<B>::Attributes ::reset()
{
    n_be = 0;
    n_fe = 0;
    n_fra = 0;
...
template<typename B>
Monitor_BFER<B>::Attributes ::Attributes()
{
    reset();
}
