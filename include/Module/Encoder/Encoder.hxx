...
template<typename B>
spu::runtime::Task&
Encoder<B>::operator[](const enc::tsk t)
{
...
template<typename B>
spu::runtime::Socket&
Encoder<B>::operator[](const enc::sck::encode s)
{
  ...
  , X_N_mem(this->n_frames)
{
    ...
    this->create_codelet(
      p,
      [ps_U_K, ps_X_N](spu::module::Module& m, spu::runtime::Task& t, const size_t frame_id) -> int
      {
          ...
          enc._encode(static_cast<B*>(t[ps_U_K].get_dataptr()), static_cast<B*>(t[ps_X_N].get_dataptr()), frame_id);
...
template<typename B>
void
Encoder<B>::set_memorizing(const bool memorizing)
{
...
template<typename B>
const std::vector<B>&
Encoder<B>::get_U_K(const size_t frame_id) const
{
...
template<typename B>
const std::vector<B>&
Encoder<B>::get_X_N(const size_t frame_id) const
{
...
template<class A>
void
Encoder<B>::encode(const std::vector<B, A>& U_K, std::vector<B, A>& X_N, const int frame_id, const bool managed_memory)
{
...
template<typename B>
void
Encoder<B>::encode(const B* U_K, B* X_N, const int frame_id, const bool managed_memory)
{
    (*this)[enc::sck::encode::U_K].bind(U_K);
    (*this)[enc::sck::encode::X_N].bind(X_N);
    (*this)[enc::tsk::encode].exec(frame_id, managed_memory);
...
template<class A>
bool
Encoder<B>::is_codeword(const std::vector<B, A>& X_N)
{
    if (this->N != (int)X_N.size())
    {
        std::stringstream message;
        message << "'X_N.size()' has to be equal to 'N' ('X_N.size()' = " << X_N.size() << ", 'N' = " << this->N
                << ").";
...
Encoder<B>::get_info_bits_pos() const
{
...
template<typename B>
void
Encoder<B>::set_sys(const bool sys)
{
    this->sys = sys;
}
...
template<typename B>
void
Encoder<B>::set_n_frames(const size_t n_frames)
{
