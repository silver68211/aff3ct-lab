...
Decoder_polar ::Decoder_polar(const std::string& prefix)
...
Decoder_polar ::Decoder_polar(const std::string& name, const std::string& prefix)
  : Decoder(name, prefix)
{
    this->type = "SC";
    this->implem = "FAST";
...
void
Decoder_polar ::get_description(cli::Argument_map_info& args) const
{
    ...
    cli::add_options(args.at({ p + "-type", "D" }), 0, "SC", "SCL", "SCL_MEM", "ASCL", "ASCL_MEM", "SCAN", "SCF", "DSCF");
...
void
Decoder_polar ::store(const cli::Argument_map_value& vals)
{
    Decoder::store(vals);
...
void
Decoder_polar ::get_headers(std::map<std::string, tools::header_list>& headers, const bool full) const
{
    ...
    if (this->type != "ML" && this->type != "CHASE")
    {
        ...
        if (this->type == "DSCF") headers[p].push_back(std::make_pair("Num. of flips", std::to_string(this->flips)));
...
Decoder_polar ::build_siso(const std::vector<bool>& frozen_bits, module::Encoder<B>* /*encoder*/) const
{
    if (this->type == "SCAN" && this->systematic)
    {
        if (this->implem == "NAIVE")
            return new module::Decoder_polar_SCAN_naive_sys<B, Q, tools::f_LLR<Q>, tools::v_LLR<Q>, tools::h_LLR<B, Q>>(
    ...
    if (!this->systematic) // non-systematic encoding
    {
        if (this->implem == "NAIVE")
        {
            if (crc != nullptr && std::unique_ptr<module::CRC<B>>(crc->clone())->get_size() > 0)
            {
                if (this->type == "SCF")
                    return new module::
                      Decoder_polar_SCF_naive<B, Q, tools::f_LLR<Q>, tools::g_LLR<B, Q>, tools::h_LLR<B, Q>>(
                        this->K, this->N_cw, frozen_bits, *crc, this->flips);
                if (this->type == "DSCF")
                    return new module::
                      Decoder_polar_DSCF_naive<B, Q, tools::f_LLR<Q>, tools::g_LLR<B, Q>, tools::h_LLR<B, Q>>(
                        this->K, this->N_cw, frozen_bits, *crc, this->flips);
                if (this->type == "SCL")
                    return new module::Decoder_polar_SCL_naive_CA<B, Q, tools::f_LLR<Q>, tools::g_LLR<B, Q>>(
                      this->K, this->N_cw, this->L, frozen_bits, *crc);
            }

            if (this->type == "SC")
                return new module::
                  Decoder_polar_SC_naive<B, Q, tools::f_LLR<Q>, tools::g_LLR<B, Q>, tools::h_LLR<B, Q>>(
                    this->K, this->N_cw, frozen_bits);
            if (this->type == "SCAN")
                return new module::Decoder_polar_SCAN_naive<B, Q, tools::f_LLR<Q>, tools::v_LLR<Q>, tools::h_LLR<B, Q>>(
                  this->K, this->N_cw, this->n_ite, frozen_bits);
            if (this->type == "SCL")
    ...
    else // systematic encoding
    {
        if (this->implem == "NAIVE")
        {
            if (crc != nullptr && std::unique_ptr<module::CRC<B>>(crc->clone())->get_size() > 0)
            {
                if (this->type == "SCF")
                    return new module::
                      Decoder_polar_SCF_naive_sys<B, Q, tools::f_LLR<Q>, tools::g_LLR<B, Q>, tools::h_LLR<B, Q>>(
                        this->K, this->N_cw, frozen_bits, *crc, this->flips);
                if (this->type == "DSCF")
                    return new module::
                      Decoder_polar_DSCF_naive<B, Q, tools::f_LLR<Q>, tools::g_LLR<B, Q>, tools::h_LLR<B, Q>>(
                        this->K, this->N_cw, frozen_bits, *crc, this->flips);
                        if (this->type == "SCL")
                    return new module::Decoder_polar_SCL_naive_CA_sys<B, Q, tools::f_LLR<Q>, tools::g_LLR<B, Q>>(
                      this->K, this->N_cw, this->L, frozen_bits, *crc);
            }

            if (this->type == "SC")
                return new module::
                  Decoder_polar_SC_naive_sys<B, Q, tools::f_LLR<Q>, tools::g_LLR<B, Q>, tools::h_LLR<B, Q>>(
                    this->K, this->N_cw, frozen_bits);
            if (this->type == "SCAN")
                return new module::
                  Decoder_polar_SCAN_naive_sys<B, Q, tools::f_LLR<Q>, tools::v_LLR<Q>, tools::h_LLR<B, Q>>(
                    this->K, this->N_cw, this->n_ite, frozen_bits);
            if (this->type == "SCL")
                return new module::Decoder_polar_SCL_naive_sys<B, Q, tools::f_LLR<Q>, tools::g_LLR<B, Q>>(
                  this->K, this->N_cw, this->L, frozen_bits);
        }
        else if (this->implem == "FAST")
        {
            ...
            if (this->type == "SC")
                decoder = new module::Decoder_polar_SC_fast_sys<B, Q, API_polar>(
                  this->K, this->N_cw, frozen_bits, polar_patterns, idx_r0, idx_r1);

            for (auto p : polar_patterns)
                delete p;
            if (decoder != nullptr) return decoder;
        }
    }
                                ...
                                module::Encoder<B>* /*encoder*/) const
{
    ...
    if (this->implem == "FAST" && this->systematic)
    {
        ...
        {
            if (this->type == "ASCL")
                decoder = new module::Decoder_polar_ASCL_fast_CA_sys<B, Q, API_polar>(
                  this->K, this->N_cw, this->L, frozen_bits, polar_patterns, idx_r0, idx_r1, *crc, this->full_adaptive);
            if (this->type == "ASCL_MEM")
                decoder = new module::Decoder_polar_ASCL_MEM_fast_CA_sys<B, Q, API_polar>(
                  this->K, this->N_cw, this->L, frozen_bits, polar_patterns, idx_r0, idx_r1, *crc, this->full_adaptive);
            if (this->type == "SCL")
                decoder = new module::Decoder_polar_SCL_fast_CA_sys<B, Q, API_polar>(
                  this->K, this->N_cw, this->L, frozen_bits, polar_patterns, idx_r0, idx_r1, *crc);
            if (this->type == "SCL_MEM")
                decoder = new module::Decoder_polar_SCL_MEM_fast_CA_sys<B, Q, API_polar>(
                  this->K, this->N_cw, this->L, frozen_bits, polar_patterns, idx_r0, idx_r1, *crc);
        }
        else
        {
            if (this->type == "SCL")
                decoder = new module::Decoder_polar_SCL_fast_sys<B, Q, API_polar>(
                  this->K, this->N_cw, this->L, frozen_bits, polar_patterns, idx_r0, idx_r1);
            if (this->type == "SCL_MEM")
                decoder = new module::Decoder_polar_SCL_MEM_fast_sys<B, Q, API_polar>(
                  this->K, this->N_cw, this->L, frozen_bits, polar_patterns, idx_r0, idx_r1);
        }
    }
...
template<typename B, typename Q>
module::Decoder_SIHO<B, Q>*
Decoder_polar ::build(const std::vector<bool>& frozen_bits,
                      const module::CRC<B>* crc,
                      module::Encoder<B>* encoder) const
{
    try
...
aff3ct::factory::Decoder_polar::build_siso<B_8, Q_8>(const std::vector<bool>&, module::Encoder<B_8>*) const;
template aff3ct::module::Decoder_SISO<B_16, Q_16>*
aff3ct::factory::Decoder_polar::build_siso<B_16, Q_16>(const std::vector<bool>&, module::Encoder<B_16>*) const;
template aff3ct::module::Decoder_SISO<B_32, Q_32>*
aff3ct::factory::Decoder_polar::build_siso<B_32, Q_32>(const std::vector<bool>&, module::Encoder<B_32>*) const;
template aff3ct::module::Decoder_SISO<B_64, Q_64>*
aff3ct::factory::Decoder_polar::build_siso<B_64, Q_64>(const std::vector<bool>&, module::Encoder<B_64>*) const;
#else
template aff3ct::module::Decoder_SISO<B, Q>*
aff3ct::factory::Decoder_polar::build_siso<B, Q>(const std::vector<bool>&, module::Encoder<B>*) const;
#endif

#ifdef AFF3CT_MULTI_PREC
template aff3ct::module::Decoder_SIHO<B_8, Q_8>*
aff3ct::factory::Decoder_polar::build<B_8, Q_8>(const std::vector<bool>&,
                                                const module::CRC<B_8>*,
                                                module::Encoder<B_8>*) const;
template aff3ct::module::Decoder_SIHO<B_16, Q_16>*
aff3ct::factory::Decoder_polar::build<B_16, Q_16>(const std::vector<bool>&,
                                                  const module::CRC<B_16>*,
                                                  module::Encoder<B_16>*) const;
template aff3ct::module::Decoder_SIHO<B_32, Q_32>*
aff3ct::factory::Decoder_polar::build<B_32, Q_32>(const std::vector<bool>&,
                                                  const module::CRC<B_32>*,
                                                  module::Encoder<B_32>*) const;
template aff3ct::module::Decoder_SIHO<B_64, Q_64>*
aff3ct::factory::Decoder_polar::build<B_64, Q_64>(const std::vector<bool>&,
                                                  const module::CRC<B_64>*,
                                                  module::Encoder<B_64>*) const;
#else
template aff3ct::module::Decoder_SIHO<B, Q>*
aff3ct::factory::Decoder_polar::build<B, Q>(const std::vector<bool>&, const module::CRC<B>*, module::Encoder<B>*) const;
#endif
// ==================================================================================== explicit template instantiation
