...
class Monitor_BFER : public Monitor
{
  ...
  private:
    ...
    const bool
      count_unknown_values; // take into account or not the unknown values as wrong values in the checked frames
  ...
  public:
    Monitor_BFER(const int K,
                 const unsigned max_fe,
                 const unsigned max_n_frames = 0,
                 const bool count_unknown_values = false);

    virtual ~Monitor_BFER() = default;

    virtual Monitor_BFER<B>* clone() const;

    bool equivalent(const Monitor_BFER<B>& m,
                    bool do_throw = false) const; // check if this monitor and "m" have equivalent construction
                                                  // arguments and then can be merged by "collect" or "copy" methods
    /*!
    ...
    int check_errors(const B* U, const B* V, const int frame_id = -1, const bool managed_memory = true);
