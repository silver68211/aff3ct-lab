...
Decoder_polar_DSCF_naive<B, R, F, G, H>::Decoder_polar_DSCF_naive(const int& K,
                                                                const int& N,
...
{
    const std::string name = "Decoder_polar_DSCF_naive";
    this->set_name(name);
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G, tools::proto_h<B, R> H>
Decoder_polar_DSCF_naive<B, R, F, G, H>*
Decoder_polar_DSCF_naive<B, R, F, G, H>::clone() const
{
    auto m = new Decoder_polar_DSCF_naive(*this);
    m->deep_copy(*this);
    return m;
}
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G, tools::proto_h<B, R> H>
void
Decoder_polar_DSCF_naive<B, R, F, G, H>::deep_copy(const Decoder_polar_DSCF_naive<B, R, F, G, H>& m)
{
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G, tools::proto_h<B, R> H>
void
Decoder_polar_DSCF_naive<B, R, F, G, H>::recursive_decode(const tools::Binary_node<Contents_SC<B, R>>* node_curr)
{
    ...
    else // specific leaf treatment
    {
        ...
        if (current_flip_index == node_curr->get_lane_id()) node_curr->get_c()->s[0] = !node_curr->get_c()->s[0];
    }
}
...
Decoder_polar_DSCF_naive<B, R, F, G, H>::_decode_siho(const R* Y_N, B* V_K, const size_t frame_id)
{
    ...
    for (auto i = 0; i < this->N; i++)
        if (!this->frozen_bits[i]) index[j++] = i;
    ...
    auto bit_metrics = this->bit_metrics(leaves);
    ...
    std::partial_sort(index.begin(),
                      index.begin() + n_flips,
                      index.end(),
                      [&bit_metrics](const int& a, const int& b)
                      { return bit_metrics[a] < bit_metrics[b]; });
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G, tools::proto_h<B, R> H>
int
Decoder_polar_DSCF_naive<B, R, F, G, H>::_decode_siho_cw(const R* Y_N, B* V_N, const size_t frame_id)
{
    // initialization
    bool decode_result = false;
    int n_ite = 0;
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G, tools::proto_h<B, R> H>
bool
Decoder_polar_DSCF_naive<B, R, F, G, H>::check_crc(const size_t frame_id)
{
    auto& leaves = this->polar_tree.get_leaves();
    ...
    for (auto leaf = 0; leaf < this->N; leaf++)
        if (!this->frozen_bits[leaf]) U_test.push_back(leaves[leaf]->get_c()->s[0]);
...
template<typename B, typename R, tools::proto_f<R> F, tools::proto_g<B, R> G, tools::proto_h<B, R> H>
std::vector<double>
Decoder_polar_DSCF_naive<B, R, F, G, H>::bit_metrics(
    const std::vector<typename Decoder_polar_DSCF_naive<B, R, F, G, H>::Node*>& leaves)
{
    std::vector<double> metrics(this->N, std::numeric_limits<double>::infinity());

    constexpr double alpha = 0.3367;
    double acc = 0.0;
    ...
    for (int i = 0; i < this->N; i++)
    {
        ...
        const double li = std::abs((double)leaves[i]->get_c()->lambda[0]);
        acc += std::log1p(std::exp(-alpha * li)) / alpha;
        metrics[i] = li + acc;
    }
